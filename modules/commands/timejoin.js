...
module.exports.run = function( {
    ...
    if (/all|list/.test(args[0])) return out(dataF.user.map((d, idx)=> `${idx+1}. ${name(d.id)}\n[⚜️]➜ Tham Gia Lúc ${(x = JSON.stringify(new Date(d.timestamp)).split(/\.|T/), `${x[1]} | ${x[0].replace(/"/, '')}`)}\n[⚜️]➜ Đã Qua ${etnTime((Date.now()+25200000)-d.timestamp)}`).join('\n\n'));
    ...
    out(`[⚜️]➜ Tên: ${name(findID.id)}\n[⚜️]➜ Tham Gia Lúc ${(x = JSON.stringify(new Date(findID.timestamp)).split(/\.|T/), `${x[1]} | ${x[0].replace(/"/, '')}`)}\n[⚜️]➜ Đã Qua ${etnTime((Date.now()+25200000)-findID.timestamp)}`);
};
