...
module.exports.config = {
    ...
    commandCategory: "Tự động", // Thuộc vào nhóm nào: system, other, game-sp, game-mp, random-img, edit-img, media, economy, ...
...
module.exports.handleEvent = async ({ api, event, Users, Threads, args }) => {
    ...
    mentionsKey.forEach(async (each) => {
        ...
        if(allAdmin.includes(each)) {
            ...
            api.sendMessage({body:`====『 𝗧𝗔𝗚 𝗔𝗗𝗠𝗜𝗡 』====\n\n👤 𝗧𝗲̂𝗻: ${userName}\n🧸 𝗡𝗵𝗼́𝗺: ${(await Threads.getInfo(threadID)).threadName || "Unknow"}\n💬 𝗡𝗼̣̂𝗶 𝗱𝘂𝗻𝗴: ${body}\n⏰ 𝗧𝗵𝗼̛̀𝗶 𝗴𝗶𝗮𝗻: ${moment().tz("Asia/Ho_Chi_Minh").format("LLLL")}\n━━━━━━━━━━━━━━━━━━\n𝗥𝗲𝗽𝗹𝘆 ( 𝗣𝗵𝗮̉𝗻 𝗵𝗼̂̀𝗶 ) 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 đ𝗲̂̉ 𝗴𝘂̛̉𝗶 𝘁𝗶𝗻 𝗻𝗵𝗮̆́𝗻 đ𝗲̂́𝗻 𝗻𝗴𝘂̛𝗼̛̀𝗶 𝗱𝘂̀𝗻𝗴 💞`, attachment: await downLoad(`https://graph.facebook.com/${event.senderID}/picture?height=720&width=720&access_token=463372798834978|csqGyA8VWtIhabZZt-yhEBStl9Y`, __dirname+'/cache/12345.jpg')},each, (err, info) => {
                if(err) console.log(err)
                else {
                    global.client.handleReply.push({
                        name: this.config.name,
