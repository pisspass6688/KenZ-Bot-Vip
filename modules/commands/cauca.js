...
module.exports.run = async function ({ event, api, args }) {
  ...
  switch ((args[0] || "").toLowerCase()) {
      ...
      default: {
          try {
              if (!existsSync(dirUser)) return api.sendMessage("[🌸 𝐅𝐢𝐬𝐡𝐢𝐧𝐠 🌸] 𝗕𝗮̣𝗻 𝗰𝗵𝘂̛𝗮 𝘁𝗵𝘂𝗲̂ 𝗰𝗮̂𝘂 𝗰𝗮́ 𝘁𝗮̣𝗶 𝗸𝗵𝘂 𝘃𝘂̛̣𝗰 𝗻𝗮̀𝘆 !\n𝐇𝐚̃𝐲 𝐠𝐡𝐢 /𝐜𝐚𝐮𝐜𝐚 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫", threadID, messageID);
              var dataUser = JSON.parse(readFileSync(dirUser, "utf-8"));
              const dateNow = moment().tz("Asia/Ho_Chi_minh");
              const format = new Intl.NumberFormat();
              if (dataUser.fishingrod.rodType == 0) return api.sendMessage("[🌸 𝐅𝐢𝐬𝐡𝐢𝐧𝐠 🌸] 𝗛𝗶𝗲̣̂𝗻 𝘁𝗮̣𝗶 𝗯𝗮̣𝗻 𝗰𝗵𝘂̛𝗮 𝗰𝗼́ 𝗰𝗮̂̀𝗻 𝗰𝗮̂𝘂, 𝗵𝗮̃𝘆 𝗺𝘂𝗮 𝗰𝗮̂̀𝗻 𝗰𝗮̂𝘂 𝗺𝗼̛́𝗶 𝘃𝗮̀ 𝘁𝗵𝘂̛̉ 𝗹𝗮̣𝗶 !\nĐ𝐞̂̉ 𝐦𝐮𝐚 𝐝𝐮̀𝐧𝐠 𝐥𝐞̣̂𝐧𝐡 /𝐜𝐚𝐮𝐜𝐚 𝐬𝐡𝐨𝐩", threadID, messageID);
